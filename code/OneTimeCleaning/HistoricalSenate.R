...
load("data/HistoricalElections/electoral_studies_replication_senate_analysis_data.Rdata", .GlobalEnv)

senate_uncleaned <- electoral_studies_senate
#don't want two copies of the same dataframe, renamed
rm(electoral_studies_senate)
...
race_facts <- senate_uncleaned %>%
  ...
  mutate(dem_incumbency = ifelse(grepl("Appointed", seat_status), 0, dem_incumbency), 
         election_year = as.numeric(election_year)) %>%
  select(c("election_year", "state", "dem_incumbency", "seat_class", "special_election")) %>%
  unique()

#cleaning senate dataset
senate_finished <- senate_uncleaned %>%
  group_by(election_year, state, special_election) %>%
